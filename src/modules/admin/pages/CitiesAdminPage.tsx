...
import { adminCitiesService, CityRecord } from '../services/adminCatalog.service';

const emptyForm = (): Omit<CityRecord, 'id'> => ({
  codigo: '',
  nombre: '',
  departamento: '',
  activo: true,
...
const columns: ColumnDef<CityRecord>[] = [
  ...
  { key: 'departamento', label: 'Departamento', width: 160 },
  { key: 'activo', label: 'Estado', width: 110 },
];

const CitiesAdminPage: React.FC = () => {
  ...
  // ── confirm toggle ──
  const [confirmRow, setConfirmRow] = useState<CityRecord | null>(null);

  const queryKey = ['admin-cities', search, activeFilter, page, pageSize];

  const { data, isLoading } = useQuery({
    queryKey,
    queryFn: () =>
      adminCitiesService.getCities({
  ...
  const createMutation = useMutation({
    mutationFn: adminCitiesService.createCity,
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: number; data: Omit<CityRecord, 'id'> }) =>
      adminCitiesService.updateCity(id, data),
    onSuccess: () => {
  ...
  const toggleMutation = useMutation({
    mutationFn: (id: number) => adminCitiesService.toggleCityStatus(id),
    onSuccess: () => {
  ...
  const openEdit = useCallback((row: CityRecord) => {
    setEditingId(row.id);
    setForm({ codigo: row.codigo, nombre: row.nombre, departamento: row.departamento, activo: row.activo });
    setModalOpen(true);
  }, []);

  const handleSave = () => {
    if (!form.codigo.trim() || !form.nombre.trim() || !form.departamento.trim()) {
      toast.error('Código, nombre y departamento son requeridos');
      return;
    }
    if (editingId !== null) {
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <Stack direction="row" alignItems="center" justifyContent="space-between" mb={3}>
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">
            Administra las ciudades del sistema
          </Typography>
        </Box>
      ...
      <Dialog open={modalOpen} onClose={() => setModalOpen(false)} maxWidth="sm" fullWidth>
        <DialogTitle>{editingId !== null ? 'Editar Ciudad' : 'Nueva Ciudad'}</DialogTitle>
      ...
      <Dialog open={!!confirmRow} onClose={() => setConfirmRow(null)} maxWidth="xs">
        ...
        <DialogContent>
          <DialogContentText>
            ¿Deseas {confirmRow?.activo ? 'desactivar' : 'activar'} la ciudad{' '}
