...
async function paginatedQuery<T>(
  ...
  if (tenantFilter) {
    query = query.eq('tenant_id', DEFAULT_TENANT);
  }
...
export const epsService = {
  ...
  createEps: async (data: Omit<EpsRecord, 'id'>) => {
    const { data: row, error } = await supabase
      .from('eps')
      .insert({ code: data.codigo, name: data.nombre, regime: data.regimen, active: data.activo, tenant_id: DEFAULT_TENANT })
      .select()
      .single();
    if (error) throw error;
    return mapEps(row);
...
export const ipsService = {
  ...
  createIps: async (data: Omit<IpsRecord, 'id'>) => {
    const { data: row, error } = await supabase
      .from('ips')
      .insert({ name: data.nombre, active: data.activo, tenant_id: DEFAULT_TENANT })
      .select()
      .single();
    if (error) throw error;
    return mapIps(row);
...
export const programasPspService = {
  ...
  createPrograma: async (data: Omit<ProgramaPspRecord, 'id'>) => {
    const { data: row, error } = await supabase
      .from('programas_psp')
      .insert({ nombre: data.nombre, descripcion: data.descripcion, activo: data.activo, tenant_id: DEFAULT_TENANT })
      .select()
      .single();
    if (error) throw error;
...
export const tiposParaclinicoService = {
  getTiposParaclinico: (params: CatalogParams = {}) =>
    paginatedQuery<TipoParaclinicoRecord>('tipos_paraclinicos', params, ['name'], mapTipoPara),
  ...
  toggleTipoParaclinicoStatus: async (id: number) => {
    const { data: cur, error: fe } = await supabase.from('tipos_paraclinicos').select('active').eq('id', id).single();
    if (fe) throw fe;
    const { data: row, error } = await supabase.from('tipos_paraclinicos').update({ active: !cur.active }).eq('id', id).select().single();
    if (error) throw error;
    return mapTipoPara(row);
  },
};
