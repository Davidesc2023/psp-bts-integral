...
const FollowupsPage = () => {
  ...
  const loadSeguimientos = useCallback(async () => {
    ...
    setErrorMsg(null);
    try {
      // TODO: Backend necesita GET /api/v1/seguimientos para el listado global entre pacientes
      const res = await seguimientoService.listar();
      setSeguimientos((res.content || []).map(mapApiToSeguimiento));
    } catch {
      setErrorMsg('No se pudo cargar la lista de seguimientos. Los seguimientos creados en esta sesión sí aparecerán.');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = {
        ...
        motivoSeguimiento: form.motivoSeguimiento,
      ...
      try {
        await supabase.from('seguimientos').insert({
          ...
          motivo_seguimiento: payload.motivoSeguimiento,
          tipo_seguimiento: payload.tipoSeguimiento,
          tipo_contacto: payload.tipoContacto,
        ...
        toast.success('✅ Seguimiento registrado (modo local)');
      }
      setSeguimientos((prev) => [{
        id: Date.now(), patientId: form.patientId!, patient: form.patientName,
        tipoSeguimiento: form.tipoSeguimiento, tipoContacto: form.tipoContacto,
        prioridad: form.prioridad, fechaProgramada: form.fechaProgramada, estado: 'PROGRAMADO',
        motivoSeguimiento: form.motivoSeguimiento, titulo: form.titulo,
        observaciones: form.observaciones, modalidad: form.modalidad,
      }, ...prev]);
      setOpenDialog(false);
      setForm(EMPTY_FORM);
    } finally {
