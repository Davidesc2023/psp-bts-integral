...
export const paraclinicoService = {
  ...
  uploadResultado: async (paraclinicId: string, file: File): Promise<string> => {
    const safe = file.name.replace(/[^a-zA-Z0-9._-]/g, '_');
    const tenantId = await getCurrentTenantId();
    const path = `${tenantId}/resultados/${paraclinicId}/${Date.now()}_${safe}`;
    const { error: upErr } = await supabase.storage
      .from('resultados-paraclinicos')
    ...
    await supabase
      .from('paraclinicos')
      .update({ url_resultado: urlData.publicUrl })
      .eq('id', paraclinicId);
