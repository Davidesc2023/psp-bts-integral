...
export const barrierService = {
  ...
  crear: async (data: BarrierRequest): Promise<Barrier> => {
    // RN1: un paciente no puede tener más de una barrera activa simultánea
    const activa = await barrierService.tieneActiva(data.patientId);
    if (activa) {
      throw new Error(
        'El paciente ya tiene una barrera activa. Cierre la barrera existente antes de crear una nueva. (RN1)'
      );
    }
