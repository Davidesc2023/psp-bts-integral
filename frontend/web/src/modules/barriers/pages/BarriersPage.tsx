...
const BarriersPage = () => {
  ...
  return (
    <Box>
                    ...
                    <Box sx={{ display: 'flex', gap: 1 }}>
                      <Tooltip title="Editar">
                        <IconButton size="small" color="primary" onClick={() => {
                          ...
                          setCreateForm({
                            patientId: barrier.patientId,
                            patientName: barrier.patientName ?? '',
                            category: barrier.category,
                            subcategory: barrier.subcategoryDescription ?? '',
                            description: barrier.description,
                            prioridad: (barrier as any).prioridad ?? 'MEDIA',
                          });
                          setOpenDialog(true);
                        }}>
                          <Edit />
      ...
      <Dialog open={openDialog} onClose={() => { setOpenDialog(false); setEditingBarrier(null); }} maxWidth="sm" fullWidth>
        <DialogTitle sx={{ fontWeight: 700, pb: 1 }}>{editingBarrier ? 'Editar Barrera' : 'Nueva Barrera'}</DialogTitle>
        <DialogContent>
        ...
        <DialogActions sx={{ px: 3, pb: 2, gap: 1 }}>
          <Button onClick={() => { setOpenDialog(false); setEditingBarrier(null); }} variant="outlined" color="inherit">Cancelar</Button>
          <Button
            onClick={async () => {
              ...
              try {
                if (editingBarrier) {
                  const updated = await barrierService.actualizar(editingBarrier.id, {
                  ...
                  setBarriersList((prev) => prev.map((b) => b.id === editingBarrier.id ? updated : b));
                  toast.success('✅ Barrera actualizada exitosamente');
                } else {
                ...
                const msg = e?.response?.data?.message ?? 'Error al guardar la barrera';
                toast.error(msg);
              } finally {
                setSaving(false);
                setOpenDialog(false);
          ...
          >
            {saving ? 'Guardando...' : editingBarrier ? 'Guardar Cambios' : 'Crear Barrera'}
          </Button>
        </DialogActions>
      </Dialog>
