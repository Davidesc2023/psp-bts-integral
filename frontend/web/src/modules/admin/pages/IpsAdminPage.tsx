...
import { ipsService, IpsRecord } from '../services/adminCatalog.service';

const emptyForm = (): Omit<IpsRecord, 'id'> => ({
  codigo: '',
  nombre: '',
  activo: true,
});

const columns: ColumnDef<IpsRecord>[] = [
  { key: 'codigo', label: 'Código', width: 120 },
  { key: 'nombre', label: 'Nombre' },
  { key: 'activo', label: 'Estado', width: 110 },
];

const IpsAdminPage: React.FC = () => {
  ...
  // ── confirm toggle ──
  const [confirmRow, setConfirmRow] = useState<IpsRecord | null>(null);

  const queryKey = ['admin-ips', search, activeFilter, page, pageSize];
  ...
  const createMutation = useMutation({
    mutationFn: ipsService.createIps,
    onSuccess: () => {
      toast.success('IPS creada correctamente');
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: number; data: Omit<IpsRecord, 'id'> }) =>
      ipsService.updateIps(id, data),
    onSuccess: () => {
      toast.success('IPS actualizada correctamente');
      queryClient.invalidateQueries({ queryKey: ['admin-ips'] });
      setModalOpen(false);
    },
  });
  ...
  const toggleMutation = useMutation({
    mutationFn: (id: number) => ipsService.toggleIpsStatus(id),
    onSuccess: () => {
  ...
  const openEdit = useCallback((row: IpsRecord) => {
    setEditingId(row.id);
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <Stack direction="row" alignItems="center" justifyContent="space-between" mb={3}>
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">
            Administra las Instituciones Prestadoras de Salud
          </Typography>
        </Box>
        <Stack direction="row" gap={1}>
          <BulkUploadButton
            catalog="ips"
            onSuccess={() => queryClient.invalidateQueries({ queryKey: ['admin-ips'] })}
          />
          <Button
            variant="contained"
            startIcon={<Add />}
      ...
      <Dialog open={modalOpen} onClose={() => setModalOpen(false)} maxWidth="sm" fullWidth>
        <DialogTitle>{editingId !== null ? 'Editar IPS' : 'Nueva IPS'}</DialogTitle>
      ...
      <Dialog open={!!confirmRow} onClose={() => setConfirmRow(null)} maxWidth="xs">
        ...
        <DialogContent>
          <DialogContentText>
            ¿Deseas {confirmRow?.activo ? 'desactivar' : 'activar'} la IPS{' '}
