...
export const EntregasPage = () => {
  ...
  const handleDevolver = (entrega: Entrega) => {
    setDevolverEntrega(entrega);
    setDevolverMotivo('');
    setOpenDevolverDialog(true);
  };

  const handleConfirmDevolver = async () => {
    if (!devolverEntrega || !devolverMotivo.trim()) return;
    try {
      await entregaService.marcarDevuelta(devolverEntrega.id, {
        motivo: devolverMotivo,
        actualizadoPor: 'USUARIO_ACTUAL',
      });
  ...
  return (
    <Box sx={{ p: 3 }}>
        ...
        <DialogContent>
          <Stack spacing={2} sx={{ mt: 2 }}>
            ...
            <Stack direction="row" spacing={2}>
              <TextField
                label="N.º de Factura de Despacho"
                value={(formData as any).facturaDespacho || ''}
                onChange={(e) => setFormData({ ...formData, ...({ facturaDespacho: e.target.value } as any) })}
                fullWidth
                placeholder="Ej: FAC-2024-00123"
              />
              <TextField
                label="N.º de Entrega"
                type="number"
                value={(formData as any).numeroEntrega || ''}
                onChange={(e) => setFormData({ ...formData, ...({ numeroEntrega: Number(e.target.value) } as any) })}
                fullWidth
                inputProps={{ min: 1 }}
                placeholder="1, 2, 3..."
              />
            ...
            <TextField
              label="Fecha Fin del Medicamento"
              type="date"
              value={(formData as any).fechaFinMedicamento || ''}
              onChange={(e) => setFormData({ ...formData, ...({ fechaFinMedicamento: e.target.value } as any) })}
              fullWidth
              InputLabelProps={{ shrink: true }}
              helperText="Fecha estimada en que se termina el medicamento actual"
            />
      ...
      {/* Dialog Motivo Devolución */}
      <Dialog open={openDevolverDialog} onClose={() => setOpenDevolverDialog(false)} maxWidth="xs" fullWidth>
        <DialogTitle sx={{ fontWeight: 700 }}>Motivo de Devolución</DialogTitle>
        <DialogContent>
          <TextField
            label="Motivo"
        ...
        <DialogActions sx={{ px: 3, pb: 2 }}>
          <Button onClick={() => setOpenDevolverDialog(false)}>Cancelar</Button>
          <Button
