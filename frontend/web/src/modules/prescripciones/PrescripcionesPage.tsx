                  ...
                  <Stack direction="row" spacing={1} justifyContent="flex-end">
                    ...
                    <IconButton
                      ...
                      onClick={async () => {
                        ...
                        try {
                          await prescripcionService.delete(prescripcion.id);
                          loadPrescripciones();
                        } catch {
                          console.error('Error eliminando prescripci\u00f3n');
                        }
                      }}
                    >
                      <DeleteIcon />
      ...
      <Dialog open={openDialog} onClose={() => setOpenDialog(false)} maxWidth="md" fullWidth>
        ...
        <DialogContent>
          <Stack spacing={2} sx={{ mt: 2 }}>
            ...
            <TextField
              label="Dosis Principal"
              value={formData.dosis || ''}
              onChange={(e) => setFormData({ ...formData, dosis: e.target.value })}
              fullWidth
              required
              placeholder="Ej: 40 mg, 2 comprimidos"
            />

            <TextField
              label="Segunda Dosis / Dosis de Carga"
              value={(formData as any).dosis2 || ''}
              onChange={(e) => setFormData({ ...formData, ...({ dosis2: e.target.value } as any) })}
              fullWidth
              placeholder="Dosis de carga o segunda dosis (si aplica)"
            />
            ...
            <TextField
              label="N.º de Prescripción MIPRES"
              value={(formData as any).numeroPrescripcionMipres || ''}
              onChange={(e) => setFormData({ ...formData, ...({ numeroPrescripcionMipres: e.target.value } as any) })}
              fullWidth
              placeholder="N.º registro MIPRES (si aplica)"
            />
            ...
            <Stack direction="row" spacing={2}>
              <TextField
                ...
                type="number"
                value={(formData as any).pesoPacienteKg || ''}
                onChange={(e) => setFormData({ ...formData, ...({ pesoPacienteKg: Number(e.target.value) } as any) })}
                fullWidth
                inputProps={{ min: 1, step: 0.1 }}
                placeholder="Ej: 70.5"
              />
              <TextField
                label="Talla del paciente (cm)"
                type="number"
                value={(formData as any).tallaPacienteCm || ''}
                onChange={(e) => setFormData({ ...formData, ...({ tallaPacienteCm: Number(e.target.value) } as any) })}
                fullWidth
                inputProps={{ min: 1 }}
                placeholder="Ej: 165"
              />
